...
public class Arm
{
    ...
    public void directKinematic(){
        ...
        if (d<2*r){
            ...
            //fix`
            double  h = Math.sqrt((Math.pow(r,2.0)) - (Math.pow((0.5*(j2-j1)),2.0)));
            double alpha = atan((yj1-yj2)/(xj2-xj1));
            // tool position
            //double xt = xa + h*cos(alpha-pi/2);
            //double yt = ya + h*sin(alpha-pi/2);
            double xt = xa + h*cos(pi/2-alpha);
            double yt = ya + h*sin(pi/2-alpha);
    ...
    public void inverseKinematic(double xt_new,double yt_new){
        ...
        double d1 = Math.sqrt(Math.Pow((xt-xm1),2.0)+(Math.Pow((yt-ym1),2.0)));
        if (d1>2*r){
        ...
        // elbows positions
        double xj21 = xa + h * cos(pi/2-alpha);
        double yj21 = ya + h * sin(pi/2-alpha);
        double xj22 = xa - h * cos(pi/2-alpha);
        double yj22 = ya - h * sin(pi/2-alpha);
        theta1 = atan2(yj21-ym2,xj21-xm2);
        ...
        //theta12 = atan2(yj22 - ym2,xj22-xm2);
        double dx2 = xt - xm2; 
        double dy2 = yt - ym2;
        double d2 = 0;
        if (d2>2*r){
        ...
        // elbows positions
        xj2 = xm2 + R * cos(theta2);
        yj2 = ym2 + R * sin(theta2);
        // motor angles for both 1st elbow positions
        theta2 = atan2(yj22 - ym2,xj22-xm2);
